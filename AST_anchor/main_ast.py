...
# Add project root directory to sys.path for importing lm_eval and other modules
# Since main_ast.py is in AST_anchor subdirectory, we need to add parent directory
project_root = Path(__file__).parent.parent
sys.path.insert(0, str(project_root))

# Add AST_anchor directory to sys.path for importing sweet_ast_anchored.py
ast_anchor_path = Path(__file__).parent
sys.path.insert(0, str(ast_anchor_path))

from lm_eval.arguments import EvalArguments
from lm_eval.evaluator_ast import EvaluatorAST  # 使用 AST 增强版本的 Evaluator
...
def parse_args():
    ...
    parser.add_argument(
        "--sweet_ast",
        action="store_true",
        help="Whether to use SWEET with AST anchors (enhanced robustness against dead code attacks)",
    )
    parser.add_argument(
        "--anchor_weight",
        type=float,
        default=0.8,
        help="AST anchor weight (0.0-1.0). Higher values mean stronger reliance on AST structure. Recommended: 0.7-0.9",
    )
    parser.add_argument(
        "--key_length",
        type=int,
...
def main():
    ...
    if args.load_generations_path or args.detect_human_code:
        ...
        if accelerator.is_main_process:
            ...
            # SWEET 和 SWEET_AST 都需要模型来计算熵值
            if args.sweet or getattr(args, "sweet_ast", False):
                if args.model != "meta-llama/Llama-2":
        ...
        evaluator = EvaluatorAST(accelerator, model, tokenizer, args)
